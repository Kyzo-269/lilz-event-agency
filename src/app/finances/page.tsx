...
export default function FinancesPage() {
  ...
  // ── Accès refusé ─────────────────────────────────────────────
  if (!isDemoMode && !checkingRole && userRole !== null && !ROLES_AUTORISES.includes(userRole)) {
    return (
  ...
  return (
    <div style={{ backgroundColor: T.bg, minHeight: "100dvh", display: "flex", flexDirection: "column" }}>
      ...
      <main style={{ flex: 1, maxWidth: "min(1600px, 94vw)", margin: "0 auto", width: "100%", padding: "20px 16px", display: "flex", flexDirection: "column", gap: 18 }}>
        ...
        {loading ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", alignItems: "start", gap: 8 }}>
            {displayed.map(entry => {
